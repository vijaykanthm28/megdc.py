...
"""
import logging
from megdc import exc
from megdc.util import versions
from megdc.hosts import ubuntu,remotes
from megdc.connection import get_connection
...
def get(hostname,
    ...
    :param use_rhmegdc: Whether or not to install RH megdc on a RHEL machine or
    ...
    module = _get_distro(distro_name, use_rhmegam=use_rhmegam)
...
def _get_distro(distro, fallback=None, use_rhmegdc=False):
    if not distro:
        return
    ...
    if distro == 'redhat' and use_rhmegdc:
        return rhel
    else:
...
def _normalized_distro_name(distro):
    distro = distro.lower()
    if distro.startswith('linuxmint'):
        return 'ubuntu'
    return distro
